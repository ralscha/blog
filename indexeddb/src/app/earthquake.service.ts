...
export class EarthquakeService {
    ...
    if (lastUpdate) {
      ...
      if (lastUpdateTs + EarthquakeService.SEVEN_DAYS < now) {
        ...
        promise = promise.then(() =>
          this.loadData('https://earthquake.usgs.gov/earthquakes/feed/v1.0/summary/all_month.csv'),
        );
      } else if (lastUpdateTs + EarthquakeService.ONE_DAY < now) {
        // database older than 1 day. load the 7 days file
        promise = promise.then(() =>
          this.loadData('https://earthquake.usgs.gov/earthquakes/feed/v1.0/summary/all_week.csv'),
        );
      } else if (lastUpdateTs + EarthquakeService.ONE_HOUR < now) {
        // database older than 1 hour. load the 1 day file
        promise = promise.then(() =>
          this.loadData('https://earthquake.usgs.gov/earthquakes/feed/v1.0/summary/all_day.csv'),
        );
      } else if (lastUpdateTs + EarthquakeService.FOURTYFIVE_MINUTES < now) {
        // database older than 45 minutes. load the 1 hour file
        promise = promise.then(() =>
          this.loadData('https://earthquake.usgs.gov/earthquakes/feed/v1.0/summary/all_hour.csv'),
        );
      }
    } else {
  ...
  filter(filter: Filter): Promise<Earthquake[]> {
    ...
    let promise = new Promise<Earthquake[]>((resolve) => {
      if (hasMagFilter && !hasTimeFilter) {
        magIndex.getAll(IDBKeyRange.bound(filter.mag.lower, filter.mag.upper)).onsuccess = (e) =>
          resolve((e.target as any).result);
      } else if (!hasMagFilter && hasTimeFilter) {
        const now = new Date();
        now.setHours(now.getHours() - parseInt(filter.time, 10));
        timeIndex.getAll(IDBKeyRange.lowerBound(now.getTime())).onsuccess = (e) =>
          resolve((e.target as any).result);
      } else if (hasMagFilter && hasTimeFilter) {
        const magPromise = new Promise<string[]>((res) => {
          magIndex.getAllKeys(IDBKeyRange.bound(filter.mag.lower, filter.mag.upper)).onsuccess = (
            e,
        ...
        const timePromise = new Promise<string[]>((res) => {
          timeIndex.getAllKeys(IDBKeyRange.lowerBound(now.getTime())).onsuccess = (e) =>
            res((e.target as any).result);
        });
        ...
        magIndex.getAll().onsuccess = (e) => resolve((e.target as any).result);
      }
    });
        ...
        e.forEach((r) => {
          const distanceInKilometers =
            getDistance(
              ...
              { latitude: filter.myLocation.latitude, longitude: filter.myLocation.longitude },
            ) / 1000;

          if (hasDistanceFilter) {
            if (
              filter.distance.lower <= distanceInKilometers &&
  ...
  private loadData(dataUrl: string): Promise<void> {
    return new Promise((resolve) => {
      const response = this.http.get(dataUrl, { responseType: 'text' }).pipe(
        map((data) =>
          parse<{
            id: string;
            time: string;
