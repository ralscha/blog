...
export class EarthquakeService {
  ...
  fetchJson(): Observable<IEarthquake[]> {
    return this.http
      .get<{ earthquakes: IEarthquake[] }>(`${environment.SERVER_URL}/earthquakes`)
      .pipe(
        map((res) => res.earthquakes),
  ...
  fetchProtobuf(): Observable<IEarthquake[]> {
    ...
    return this.http
      .get(`${environment.SERVER_URL}/earthquakes`, { headers, responseType: 'arraybuffer' })
      .pipe(
